...
test.describe('Subscription Management', () => {
  ...
  test('should validate subscription features access', async ({ page }) => {
    ...
    // Verify premium features are highlighted
    await expect(subscriptionPage.premiumFeatures).toBeVisible();
    await expect(subscriptionPage.currentPlanBadge).toContainText('Current Plan');

    // Navigate to features that require subscription
    await settingsPage.goto();
    await settingsPage.clickDevices();

    // Verify premium features are accessible
    await expect(settingsPage.locationTrackingSection).toBeVisible();
    await expect(settingsPage.locationTrackingSection).not.toContainText('PRO');

    // Test zone management access
    await settingsPage.clickZones();
    await expect(settingsPage.zoneManagement).toBeVisible();
    await expect(settingsPage.createZoneButton).toBeEnabled();
  });
  ...
  test('should update subscription status in settings', async ({ page }) => {
    ...
    const subscriptionItem = page.locator('[data-testid="setting-subscription"]');
    await expect(subscriptionItem).toBeVisible();

    // Verify status is shown in settings
    await expect(subscriptionItem.locator('[data-testid="setting-subtitle"]')).toContainText('DAMP+ Active');
  });
});