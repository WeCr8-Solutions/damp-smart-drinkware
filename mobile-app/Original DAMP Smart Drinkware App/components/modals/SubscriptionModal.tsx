...
export default function SubscriptionModal({
    ...
    return (
      <View key={product.priceId} style={[
        ...
        <TouchableOpacity
          ...
          testID={`subscribe-button-${product.id.split('_').pop()?.toLowerCase()}`}
        >
          {isLoadingThis ? (
    ...
    return (
      ...
      >
        <View style={settingsStyles.loadingContainer}>
          ...
          <Text style={settingsStyles.loadingText}>Loading subscription information...</Text>
        </View>
      </BaseModal>
    );
  }
  ...
  return (
    <BaseModal
      ...
      title="Subscription"
    >
      <ScrollView showsVerticalScrollIndicator={false} testID="subscription-page">
        ...
        {error ? (
          <View style={styles.errorContainer} testID="network-error">
        ...
        {userSubscription && (
          <View style={styles.currentSubscriptionCard} testID="current-subscription">
            <CreditCard size={24} color="#0277BD" />
            <View style={styles.subscriptionInfo}>
              <Text style={styles.subscriptionTitle}>Current Subscription</Text>
              <Text style={styles.subscriptionStatus} testID="subscription-status">
                {formatSubscriptionStatus(userSubscription.subscription_status)}
                {userSubscription.current_period_end && (
                  <Text style={styles.subscriptionPeriod} testID="next-billing-date">
                    {' • Renews '}
              ...
              {getCurrentProduct() && (
                <Text style={styles.currentPlanName} testID="subscription-plan">
